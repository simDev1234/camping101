...
- 해결과정 : 
  ...
  왜 운영체제는 이러한 권한을 부여하는지, 리눅스 파일 시스템 구조는 어떤지 알아보고, 리눅스에 사용자를 추가해 권한 부여를 하여 문제 해결을 해보고 싶었다.<br><br>
...
- 원인 : AWS EC2에서 사용할 수 있는 메모리 용량이 한정되어 있는데, 배포 시 사용하는 용량이 그보다 높아 느려지는 현상이었다. 
- 해결 : 처음에는 원인을 알 수 없었는데 프리티어 용량을 확인해보니 메모리가 1GB / 스토리지 30GB 밖에 되지 않는다는 걸 보았고, 실제로 AWS CPU 사용률을 보니 100%에 가까웠다. 
  무료를 포기할 수가 없어서 Swap 파일을 생성해서 메모리 이슈를 해결해 보는 방안을 찾아보았다. (그러나 여전히 CPU는 제자리였다.) 
  프론트에서 API 테스트를 해야했기 때문에 일단은 ngrok을 사용해서 local PC에 접속할 수 있도록 하였다. 매일마다 ngrok을 키고 로컬 서버를 돌려서 전달해야했기에 불편함이 있었다.
  결국 회의를 거쳐 팀원들과 함께 협의해 약간의 비용을 주더라도 서버를 업그레이드하여 편안하게 배포를 하기로 했고, 이후 매우 편안하게 서버 배포가 가능해졌다. <br>

<br>

## ISSUE 4 : Security ProviderNotFoundException for UsernamePasswordAuthenticationToken
- 원인 : SpringSecurity의 AuthenticationProvider를 확장한 'UsernamePasswordAuthenticationProvider'클래스의 supports 메소드에 'UsernamePasswordAuthenticaionToken'을 지정해주지 않아서 나타난 이슈였다. 일단 SpringSecurity의 기본 UsernamePassword 인증 절차를 풀이하면 다음과 같았다. <br>
  ```bash
...
## ISSUE 5 : 프론트에서 PUT 메소드로 요청 시 500 에러가 발생
